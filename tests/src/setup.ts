...
export async function loadContractAndCallConstructor(name: string, idlPath: string, args: any[] = [], space: number = 8192):
    Promise<{ program: Program, payer: Keypair, provider: AnchorProvider, storage: Keypair, program_key: PublicKey }> {

    const {program, payer, provider, program_key} = await loadContract(name, idlPath);
...
export async function loadContract(name: string):
    ...
    const program = new Program(idl, provider.wallet.publicKey, provider)
